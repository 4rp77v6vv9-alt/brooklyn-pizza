...
const defaults={
  ...
  hardware:{
    ...
    printer:{enabled:false,provider:'system'},
...
function merge(a,b){
  const hw=b?.hardware||{}
  return {...a,...(b||{}),hardware:{
    fiscal:{...a.hardware.fiscal,...(hw.fiscal||{})},acquiring:{...a.hardware.acquiring,...(hw.acquiring||{})},
    scanner:{...a.hardware.scanner,...(hw.scanner||{})},printer:{...a.hardware.printer,...(hw.printer||{})},
    cashDrawer:{...a.hardware.cashDrawer,...(hw.cashDrawer||{})},egais:{...a.hardware.egais,...(hw.egais||{})},marking:{...a.hardware.marking,...(hw.marking||{})}
  }}
}
...
function publicView(value=read()){
  const c=structuredClone(value);if(c.hardware?.marking)c.hardware.marking.token=c.hardware.marking.token?'••••••••':'';return c
}
