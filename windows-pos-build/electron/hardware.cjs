...
function adapter(setting,path,payload){if(!setting?.enabled||!setting?.adapterUrl)return Promise.resolve(notConfigured('Локальный адаптер'));return request(join(setting.adapterUrl,path),{method:'POST',body:payload||{},token:setting.token,timeout:8000})}
...
async function status(){
  ...
  return {ok:true,
    fiscal:{...c.hardware.fiscal,status:c.hardware.fiscal.enabled&&c.hardware.fiscal.adapterUrl?await adapter(c.hardware.fiscal,'v1/fiscal/status',{}):{ok:false,disabled:!c.hardware.fiscal.enabled}},
    scanner:{...c.hardware.scanner},
    printer:{...c.hardware.printer},
    kitchenScreens:c.hardware.kitchenScreens||[],
    cashDrawer:{...c.hardware.cashDrawer},
...
async function command(channel,payload={}){
  ...
  switch(channel){
    ...
    case 'egais.discrepancy':return c.hardware.egais.enabled&&c.hardware.egais.adapterUrl?adapter(c.hardware.egais,'v1/egais/discrepancy',payload):notConfigured('Адаптер ЕГАИС/УТМ')
    ...
    case 'egais.resend':return c.hardware.egais.enabled&&c.hardware.egais.adapterUrl?adapter(c.hardware.egais,'v1/egais/resend',payload):notConfigured('Адаптер ЕГАИС/УТМ')
    ...
    case 'egais.taps':return c.hardware.egais.enabled&&c.hardware.egais.adapterUrl?adapter(c.hardware.egais,'v1/egais/taps',payload):notConfigured('Адаптер ЕГАИС/УТМ')
